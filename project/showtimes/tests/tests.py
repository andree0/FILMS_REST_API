...
from .utils import fake_cinema_data, fake_screening_data, TZ
...
def test_update_cinema(client, set_up):
    ...
    response = client.patch(f"/cinemas/{cinema.id}/",
                            cinema_data, format='json')
    assert response.status_code == 200
...
def test_update_screening(client, set_up):
    ...
    new_movie = choice(movies).title
    screening_data["movie"] = new_movie
    new_date = faker.future_datetime(end_date='+30d', tzinfo=TZ)
    ...
    assert screening_obj.movie.title == new_movie
    assert screening_obj.date == new_date
